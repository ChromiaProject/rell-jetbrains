...
fun PsiElement.isRule(ruleIndex: Int): Boolean =
    elementType == RellPsiElementTypes.rule(ruleIndex)

/** True if this element is an ANTLR token leaf of the given token type. */
fun PsiElement.isToken(antlrTokenType: Int): Boolean =
    elementType == RellPsiElementTypes.token(antlrTokenType)
