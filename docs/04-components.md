...
`RellSyntaxHighlighter` (lexer-level coloring), `RellFoldingBuilder`, `RellPairedBraceMatcher`,
`RellCommenter`, `RellQuoteTokenHandler` and `RellSpellcheckingStrategy` all navigate the generated
rule/token indices rather than hard-coded IDs.

Semantic coloring is the language server's job, not the PSI's: `RellSemanticTokensSupport` maps the
server's semantic tokens onto the `RellColor` palette.
...
Formatting and completion themselves come from the language server over LSP.
